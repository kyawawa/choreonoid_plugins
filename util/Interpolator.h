...
/*
  ...
  when Initial Jerk = 0
*/
minJerkCoeffTime calcMinJerkCoeffTime(const Eigen::Vector3d& start, const Eigen::Vector3d& finish);

// double calcFifthOrderSpline(double t, const minJerkCoeff& coeff);

template<size_t n>
double calcNthOrderSpline(const minJerkCoeff& coeff, double t);
